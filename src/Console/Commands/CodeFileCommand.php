...
class CodeFileCommand extends Command
{
    ...
    public function handle(GenerateHtmlReport $generator): int
    {
        try {
            ...
            if ($outputArg !== null) {
                if (is_dir($outputArg) || str_ends_with($outputArg, '/')) {
                    $dir = rtrim($outputArg, '/');
                    if (! is_dir($dir)) {
                        mkdir($dir, 0755, true);
                    }
                    $outputArg = $dir.'/'.pathinfo($relPath, PATHINFO_FILENAME).'.html';
                } elseif (! is_dir(dirname($outputArg))) {
                    mkdir(dirname($outputArg), 0755, true);
                }
